...
module.exports = {
        ...
        run: async (client, message, args, cmd) => {
            ...
            if (args[0] === "on") { db.set(`${message.guild.id}.anti.roleupdate`, "on") } else
                if (args[0] === "off") { db.delete(`${message.guild.id}.anti.roleupdate`) } else
                    if (args[0] === "max") { db.set(`${message.guild.id}.anti.roleupdate`, "max") } else return message.reply(`L'antiroleupdate est sur \`${db.fetch(`${message.guild.id}.anti.roleupdate`) || "off"}\` !`)
