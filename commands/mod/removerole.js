...
module.exports = {
  name: "removerole",
  description: "Retire un rôle au membre mentionné",
  aliases: ["remove-role", "delrole"],
  ...
  run: async (client, message, args, cmd) => {
    ...
    if (perm == true) {
      ...
      if (!member.roles.cache.has(role.id)) return message.reply(`:x: le membre n'a pas ce rôle`)
      member.roles.remove(role).catch(e => { message.channel.send(e) })
      message.reply(`**${member.user.username}** a perdu le rôle \`${role.name}\` !`)
      ...
      if (logchannel) logchannel.send({
        embeds: [new Discord.MessageEmbed()
          ...
          .setDescription(`${message.author} a **retiré le rôle** ${role} à ${member}`)]
      }).catch(e => { e })
